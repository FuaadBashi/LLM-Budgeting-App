...
| ~~**`Account.default_category_id`**~~ | **Shipped 1 September 2026.** Migration 0008; stamped at write time by `categories.apply_account_defaults`; refused with a 422 on any non-expense account, because it is only ever read while stamping an expense leg; backfilled only by `scripts/backfill_categories.py --apply`, since rewriting how a closed period was categorised should take a decision rather than be a side effect. *Original entry:* Fixes loan interest, bank fees and untagged rent landing in the discretionary bucket. Needs a schema column, a write-time stamping path and a backfill. £50 of contractually unavoidable interest consuming 8.3% of a £600 discretionary budget is real but not blocking; Phase 4, alongside the category work. |
